...
当前代码对应版本 `v0.8.2`：已包含 Cloudflare Worker、D1、GitHub/Google OAuth、用户隔离、多故事存档、加密 `.aitown` 导入导出、角色和词库编辑、DeepSeek 生成下一天、故事历史、一天 7 个 scene beat 的时间线、React 前端架构、浮层故事侧栏，以及使用本地 tileset 和 spritesheet 文件渲染的 AI Town 风格像素地图。0.8.2 加入根据视口动态计算的拖动边界，并将不匹配的大块物件裁切替换为更贴合地图尺度的小像素地标。自动生成新角色会在后续版本补上。
