...
The current code is version `v0.6.0`: Cloudflare Worker, D1, GitHub/Google OAuth, user isolation, multiple story saves, encrypted `.aitown` import/export, character and lexicon editing, DeepSeek next-day generation, story history, a 7-beat daily timeline, a beat-driven town map, and a React front-end architecture. Automatic new character generation and the pixel-art map upgrade are planned for later versions.
