...
The current code is version `v0.7.0`: Cloudflare Worker, D1, GitHub/Google OAuth, user isolation, multiple story saves, encrypted `.aitown` import/export, character and lexicon editing, DeepSeek next-day generation, story history, a 7-beat daily timeline, a React front-end architecture, and a pixel-style town map with sprite-like character presentation. Automatic new character generation is planned for a later version.
